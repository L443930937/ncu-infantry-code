...
void gimbal_pid_init(void)
{
  ...
  //大神符模式
   PID_struct_init(&pid_pit_dashen, POSITION_PID, 5000, 1000,
                  4.0f, 0.03f, 2.0f);  //4.0 0.05 0.5
   PID_struct_init(&pid_pit_dashen_spd, POSITION_PID, 5000, 1000,
                  1.5f, 0.0f, 0.0f );
  
	 PID_struct_init(&pid_yaw, POSITION_PID, 5000, 300,
                  4.0f, 0.01f, 25.0f); // 6.0 0.05 5.0
  ...
  xLastWakeTime = xTaskGetTickCount();	
	...
	for(;;)		
    {
			...
			if(pit_get.angle>7770 )
      {
         ...
         pit_set.expect_remote = 7750-pit_get.offset_angle;
      }else if(pit_get.angle<6000 )
      {
