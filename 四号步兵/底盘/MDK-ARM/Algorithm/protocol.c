...
void sendata(void)
{
  ...
  custom_data_t.sender_id = Robot.id;//机器人id
  if(Robot.id<10)//红方
  custom_data_t.cilent_id = red_Infantry_3;//客户端id
  else //蓝方
...
void Referee_Data_Handler(void)
{
		...
		for(uint8_t num = 0; num < USART6_RX_NUM; num++)
		{
            ...
            {
//              Robot.heat.shoot_17_cooling_rate = 0;
//              Robot.heat.shoot_42_cooling_rate = 0;
            }
